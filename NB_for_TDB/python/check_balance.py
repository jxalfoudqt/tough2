...
title = 'NB_for_TDB.listing'
# #--- post-process the output ---------------------------
lst = t2listing(title)
dat = t2data('NB_for_TDB')
...
vapor_adv_xt_mtx_kgPs          = vapor_mass_fraction_in_gas_xt_mtx[0]*gas_flow_xt_mtx_kgPs
  
print('Check mass balance\n')  
# ---Mass Balance---
element_volume_m3                 = np.array([blk.volume for blk in dat.grid.blocklist])
liquid_mass_element_kg            = np.array([element_volume_m3[0]*liq_density_xt_mtx_kgPm3[0,i]*liq_saturation_xt_mtx[0,i]*dat.grid.rocktype['SAND '].porosity for i in range(lst.num_times)])
liquid_mass_kg                    = liquid_mass_element_kg
vapor_mass_element_kg             = np.array([vapor_mass_fraction_in_gas_xt_mtx[0,i]*element_volume_m3[0]*gas_density_xt_mtx_kgPm3[0,i]*gas_saturation_xt_mtx[0,i]*dat.grid.rocktype['SAND '].porosity for i in range(lst.num_times)])
vapor_mass_kg                     = vapor_mass_element_kg
...
total_water_net_flux_kgPs                  = liquid_flow_top1_kgPs+vapor_flow_top1_kgPs
