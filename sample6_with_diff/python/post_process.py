...
while i<lst.num_times:
    ...
    #plt.ticklabel_format(style='sci', axis='x', scilimits=(0,0))	
    ax4=ax3.twiny()	
    ax4.plot(Vapor_diff_flow_raw[:,i]*10,connection_value,'r1-')
    ...
    ax7.set_xscale('log')
    ax8 = ax7.twinx() 
    ax8.plot(lst.times[:i+1]/day2s,Vapor_flow_adv_topsoil[:i+1],'r1-',)
    plt.ylim(-1.3e-1,-6e-2)
    #plt.ylim(np.min(Vapor_flow_adv_raw),np.max(Vapor_flow_adv_raw))
    plt.xlim(0,np.max(lst.times)/day2s)
    plt.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
    ...
    ax7=plt.subplot(616)
    ax7.plot(lst.times[:i+1]/day2s,Vapor_flow_topsoil[:i+1],'k1-')
