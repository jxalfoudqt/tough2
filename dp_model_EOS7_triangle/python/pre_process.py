...
dat.incon[str(dat.grid.blocklist[-1])] = \
         [None, [p_atm_pa-liquid_density_kgPm3*dat.parameter['gravity']*(dat.grid.blocklist[-1].centre[2]+mean_water_level_m), 1, 10.01, T_initial]]
for i in range(nblks_z-1):
...
dat.write(dat.title)
print("file " + dat.title +" generated\n")
...
# depth_variation_m      = np.cos(np.linspace(0,100*np.pi,len(time_variation_s)))*0.5
# first_primary_variable = P_initial-liquid_density_kgPm3*dat.parameter['gravity']*(dat.grid.blocklist[-1].centre[2]+mean_water_level_m+depth_variation_m)
