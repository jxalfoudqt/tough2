...
while i<lst.num_times:
    ...
    ax3=plt.subplot(443)
    ax3.plot(vapor_diff_flow_mmPday[:,i]*1.e6,connection_value,'r1-')
    ...
    ax3=plt.subplot(444)
    ax3.plot(vapor_mass_fraction_in_gas[:,i]*10,element_value,'k1-')
    plt.xlabel('Mass_Vapor/Mass_Gas ($*10^-1$)')
    # plt.ylabel('high (m)')
    plt.ylim(1.1,-0.1)
    #plt.xlim(1,1.6)
    plt.xlim(np.min(vapor_mass_fraction_in_gas*10),np.max(vapor_mass_fraction_in_gas*10))
    #ax3.set_yscale('log')
    #plt.ticklabel_format(style='sci', axis='x', scilimits=(0,0))	
    ax4=ax3.twiny()
    ax4.plot(capillary_pressure_pa[:,i]*kpaPpa,element_value,'k1-')
    ...
    ax7=plt.subplot(717)
    ax7.plot(lst.times[:i+1]*dayPs,water_generation_vapor_mmPday[:,:i+1][0],'b1-')
