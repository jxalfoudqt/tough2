...
while i<lst.num_times:
    ...
    ax3=plt.subplot(444)
    ax3.plot(vapor_mass_fraction_in_gas[:,i]*100,element_value,'k1-')
    plt.xlabel('Vapor mass fraction (%)')
    # plt.ylabel('high (m)')
    plt.ylim(1.1,-0.1)
    #plt.xlim(1,1.6)
    plt.xlim(np.min(vapor_mass_fraction_in_gas*100),np.max(vapor_mass_fraction_in_gas*100))
    #ax3.set_yscale('log')
    #plt.ticklabel_format(style='sci', axis='x', scilimits=(0,0))	
    ax4=ax3.twiny()	
