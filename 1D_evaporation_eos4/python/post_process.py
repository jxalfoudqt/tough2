...
while i<lst.num_times:
    ...
    ax1=plt.subplot(442)
    ax1.plot(gas_pressure_pa[:,i]*paPkpa, element_value,'k1-')
    plt.xlabel('Gas Pressure (Kpa)')
    #ax1.spines['top'].set_color('red')
    plt.ylim(1.5,-0.1)
    #plt.xlim(100,115)
    plt.xlim(np.min(gas_pressure_pa*paPkpa),np.max(gas_pressure_pa*paPkpa))
    #plt.ticklabel_format(style='sci', axis='x', scilimits=(0,0))
    #ax1.set_yscale('log')
    ax2=ax1.twiny()
    ...
    ax3=plt.subplot(443)
    ax3.plot(capillary_pressure_pa[:,i]*paPkpa,element_value,'k1-')
    plt.xlabel('Capillary Pressure (Kpa)')
    # plt.ylabel('high (m)')
    plt.ylim(1.5,-0.1)
    #plt.xlim(-50,0.1)
    plt.xlim(np.min(capillary_pressure_pa*paPkpa),np.max(capillary_pressure_pa*paPkpa))
    #ax3.set_yscale('log')
    #plt.ticklabel_format(style='sci', axis='x', scilimits=(0,0))	
    ax4=ax3.twiny()	
    ...
    ax3=plt.subplot(444)
    ax3.plot(vapor_pressure_pa[:,i]*paPkpa,element_value,'k1-')
    plt.xlabel('Vapor Pressure (Kpa)')
    # plt.ylabel('high (m)')
    plt.ylim(1.5,-0.1)
    #plt.xlim(1,1.6)
    plt.xlim(np.min(vapor_pressure_pa*paPkpa),np.max(vapor_pressure_pa*paPkpa))
    #ax3.set_yscale('log')
    #plt.ticklabel_format(style='sci', axis='x', scilimits=(0,0))	
    ax4=ax3.twiny()	
    ax4.plot(vapor_diff_flow_mmPday[:,i]*1.e6,connection_value,'r1-')
    plt.xlabel('Vapor Diffusion ($*10^-6$ mm/day)')
    # plt.ylabel('high (m)')
    plt.ylim(1.5,-0.1)
    #plt.xlim(-0.1,1.3)
    plt.xlim(np.nanmin(vapor_diff_flow_mmPday)*1.e6,np.nanmax(vapor_diff_flow_mmPday)*1.e6)	
    #plt.ticklabel_format(style='sci', axis='x', scilimits=(0,0))
    ax4.spines['top'].set_color('red')	
    ax4.xaxis.label.set_color('red')
    ax4.tick_params(axis='x', colors='red')	 	
    ...
    ax7=plt.subplot(817)
    ax7.plot(lst.times[:i+1]/day2s,water_generation_vapor_mmPday[:,:i+1][0],'k1-')
    plt.ylabel('Vapor generation\n(mm/day)')
    #plt.xlabel('Time (day)')
    #plt.ylim(-0.1e-3,0.11e-2)
    plt.ylim(np.min(water_generation_vapor_mmPday),np.max(water_generation_vapor_mmPday))
    plt.xlim(0,np.max(lst.times)/day2s)
    plt.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
    #ax7.set_xscale('log')
    ax8 = ax7.twinx() 
    ax8.plot(lst.times[:i+1]/day2s,water_generation_liquid_mmPday[:,:i+1][0],'r1-',)
    #plt.ylim(-1.e1,1.1e2)
    plt.ylim(np.min(water_generation_liquid_mmPday),np.max(water_generation_liquid_mmPday))
    plt.xlim(0,np.max(lst.times)/day2s)
    plt.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
    ...
    ax7=plt.subplot(818)
    ax7.plot(lst.times[:i+1]/day2s,water_generation_mmPday[:,:i+1][0],'k1-')
    plt.ylabel('water generation (mm/day)')
    plt.xlabel('Time (day)')
    #plt.ylim(-0.1e-3,0.11e-2)
    plt.ylim(np.min(water_generation_mmPday),np.max(water_generation_mmPday))
    plt.xlim(0,np.max(lst.times)/day2s)
    #plt.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
